    ...
    this.policyImprovement = function(){
      return new Promise( resolve=>{
        ...
        for (let state = 0; state < _nStates; state += 1) {
          ...
          if (isPolicyStable) {
            console.log('yes its best')
          }else{
            console.log('policy is unstable ')
          }
    ...
    this.run = async (nEpisodes = 100)=>{
      ...
      return new Promise(async (resolve)=>{
        ...
        while (true) {
          ...
          console.log('callback finished', terminate);

          if (isPolicyStable){
            console.log('policy is stable', this.getPolicy(), _model.valueFns)
